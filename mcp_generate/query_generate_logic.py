...
3. Incorporating complex planning elements **where appropriate**, such as dynamic execution paths (if-then scenarios) or information integrity checks.
...
- Logic & Sequence (e.g., "If tool A results exceed X, pivot to plan B", "Step 2 must use verified output from Step 1")
- Data Integrity (e.g., "No data older than 24 hours", "Must cross-check price from 2 different sources")
- Accessibility/Ethics (e.g., "Must follow WCAG 2.1 accessibility standards", "Ensure all recommendations are gender-neutral")
- Select a mix of constraints that best fits the natural flow of the scenario; not every query requires logical branching if the planning complexity is already high.
...
- Handle conditional branching based on tool outputs
...
Example 4 (Market Research - dynamic planning): 
Query: "I am evaluating a position in Nvidia (NVDA) for Q1 2026. First, retrieve the current price of NVDA and cross-verify it across two different financial data sources to ensure the margin of error is below 0.2% and the data is no older than 15 minutes. If the price shows volatility greater than 5% from the daily open or if recent news sentiment is over 40% negative, pivot immediately to a competitive analysis of Advanced Micro Devices (AMD) instead. Total research budget is capped at $2,000. I prefer higher data accuracy over response speed, and please minimize total tool calls where possible to conserve our API quota."

Hard Constraints:

- {{"type": "data_integrity", "description": "Must cross-verify price from two sources with <0.2% error and <15 min freshness"}}
- {{"type": "logic_sequence", "description": "If NVDA volatility > 5% or sentiment is negative, pivot immediately to AMD analysis"}}
- {{"type": "budget_ceiling", "description": "Total costs for research and data retrieval cannot exceed $2,000"}}

Soft Constraints:
- {{"type": "quality_preference", "description": "Prioritize high-accuracy data and deep analysis over fast response times"}}
- {{"type": "cost_optimization", "description": "Minimize total tool calls and data requests to preserve API rate limits"}}
- {{"type": "risk_mitigation", "description": "Prefer established financial data providers over newer, unverified sources"}}
...
- Logic & Workflow: conditional_pivot, fallback_strategy, dependency_verification, outcome_branching, sequence_lock
- Data Trust & Integrity: consensus_requirement, freshness_threshold, cross_source_validation, audit_trail_requirement
- Ethical & Social: inclusive_design, accessibility_audit, bias_mitigation, neutrality_standard
...
8. Verify the query presents a genuine planning challenge. While logical branching (if-then) is highly encouraged for advanced scenarios, ensure it only appears where it feels natural to the user's intent.
